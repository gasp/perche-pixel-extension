...
function stringToRgb(color: string): RGB {
  if (color.startsWith('rgb(')) {
    return (color.match(/\d+/g) || ['0', '0', '0']).map(Number) as RGB
  }
...
export const gridPixelToWplaceColor = (
  gridPixel: { x: number; y: number; color: string },
  tile: { x: number; y: number },
): WplaceColor => {
  const { x, y, color } = gridPixel
  const { x: tileX, y: tileY } = tile
  const [r, g, b] = stringToRgb(color)
  const metaColor = colors
    .filter(c => c.id !== 0) // Skip transparent color
    .find(c => c.rgb[0] === r && c.rgb[1] === g && c.rgb[2] === b)
  if (!metaColor) {
    throw new Error(`Color not found: ${color}`)
  }
  return {
    key: `t=(${tileX},${tileY});p=(${x},${y});s=0`,
    value: {
      color: { r, g, b, a: 255 },
      tile: [tileX, tileY],
      pixel: [x, y],
      season: 0,
      colorIdx: metaColor.id,
    },
  }
}
